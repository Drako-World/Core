...
const char * const BITCOIN_CONF_FILENAME = "drakocoin.conf";
...
#else
    const char* pszModule = "drakocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DrakoCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/DrakoCoin";
#else
    // Unix-like
    return pathRet / ".drakocoin";
#endif
#endif
}
