...
The DrakoCoin Core daemon can be configured to act as a trusted "border
router", implementing the drakocoin wire protocol and relay, making
...
The ZeroMQ feature in DrakoCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building drakocoind:
...
For instance:
    ...
    $ drakocoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/drakocoind.tx.raw \
...
These options can also be provided in drakocoin.conf.
...
From the perspective of drakocoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into drakocoind directly. Furthermore, no information is
...
using. DrakoCoind appends an up-counting sequence number to each
