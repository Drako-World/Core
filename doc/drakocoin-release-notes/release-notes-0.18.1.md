DrakoCoin Core version 0.18.1 is now available from:

  <https://download.drakocoin.org/drakocoin-0.18.1/>
...
`/Applications/DrakoCoin-Qt` (on Mac) or `drakocoind`/`drakocoin-qt` (on
Linux).
...
DrakoCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use DrakoCoin Core on unsupported systems.
...
10.10. Additionally, DrakoCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:22429:22429` (this is an extra `:22429` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@drakocoin.org) the DrakoCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 DRKC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in DRKC per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/drakocoin-conf.md)
  about the `drakocoin.conf` file describes how to use it to configure
  DrakoCoin Core.

- A new document introduces DrakoCoin Core's BIP174 [Partially-Signed
  DrakoCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent DrakoCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  DrakoCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when DrakoCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of DrakoCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart drakocoind with the `-deprecatedrpc=generate`
...
- A new `drakocoin-wallet` tool is now distributed alongside DrakoCoin
...
This section describes planned changes to DrakoCoin Core that may affect
other DrakoCoin software and services.

- Since version 0.16.0, DrakoCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with DrakoCoin Core 0.20 (expected about
  a year after 0.18), DrakoCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the DrakoCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in DrakoCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any DrakoCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, DrakoCoin Core
...
- A sub-project of DrakoCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with DrakoCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to DrakoCoin Core's own implementation, although entropy
  gathered by DrakoCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves DrakoCoin Core a
...
- On macOS, DrakoCoin Core now opts out of application CPU throttling
...
- #22429 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
