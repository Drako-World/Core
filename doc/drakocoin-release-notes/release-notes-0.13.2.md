DrakoCoin Core version 0.13.2 is now available from:

  <https://download.drakocoin.org/drakocoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a drakocoin
...
ECDSA signatures inside DrakoCoin transactions now use validation using
...
Previous versions of DrakoCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of DrakoCoin Core was to raise the default minimum
relay fee.

DrakoCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
DrakoCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running drakocoind without having to do any manual configuration.
...
DrakoCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. DrakoCoin Core will relay transactions with
...
entirely in DrakoCoin Core 0.13, and it is at this time undecided whether the
...
DrakoCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
DrakoCoin Core automatically creates a hidden service to listen on, without
manual configuration. DrakoCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if DrakoCoin Core is listening, and
...
DrakoCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals DrakoCoin
Core to use floating fees. By default, DrakoCoin Core will use floating
fees.
...
(default: `0.0002` DRKC/kB).

At all times, DrakoCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) DRKC.
Furthermore, DrakoCoin Core will never create transactions paying less than
...
`drakocoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 222429 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 222429 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /drakocoinrpc>
        ProxyPass http://127.0.0.1:22429/
        ProxyPassReverse http://127.0.0.1:22429/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass drakocoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< drakocoinrpc:<password>
    </Location>
...
Restarting drakocoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in drakocoin.conf
...
drakocoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/drakocoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The DrakoCoin Core code base has
...
- `drakocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `drakocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the DrakoCoin scripting system that in combination with
...
DrakoCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of DrakoCoin Core.
...
The two phases are distinct in the DrakoCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming DrakoCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for DrakoCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each DrakoCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of drakocoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of drakocoins being spent is exactly
...
- **Increased security for multisig:** DrakoCoin addresses (both P2PKH addresses
  ...
  believed to be provided by DrakoCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of DrakoCoin's current ledger (called the UTXO set),
  ...
  note that DrakoCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  DrakoCoin users to individually opt-in to almost any change in the DrakoCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and DrakoCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the DrakoCoin
client to DrakoCoin Core 0.13.2 to signal for segwit support. When a
...
Since DrakoCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `drakocoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `drakocoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
